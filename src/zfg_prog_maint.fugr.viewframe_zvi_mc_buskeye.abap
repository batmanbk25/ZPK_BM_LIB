FUNCTION VIEWFRAME_ZVI_MC_BUSKEYE.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_MC_BUSKEYE
*   generation date: 21.04.2015 at 17:18:24 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSKEYE'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_MC_BUSKEYE_EXTRACT
TOTAL = ZVI_MC_BUSKEYE_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSKEYE'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_MC_BUSKEYE-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_MC_BUSKEYE_EXTRACT
TOTAL = ZVI_MC_BUSKEYE_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_MC_BUSKEYE-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSKEYE'
...
TOTAL = ZVI_MC_BUSKEYE_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_MC_BUSKEYE-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_MC_BUSKEYE-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSKEYE'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_MC_BUSKEYE-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_MC_BUSKEYE_EXTRACT
TOTAL = ZVI_MC_BUSKEYE_TOTAL
