FUNCTION VIEWFRAME_ZVI_FIELD_DB.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_FIELD_DB
*   generation date: 21.05.2014 at 15:51:01 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DB'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_FIELD_DB_EXTRACT
TOTAL = ZVI_FIELD_DB_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DB'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_FIELD_DB-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_FIELD_DB_EXTRACT
TOTAL = ZVI_FIELD_DB_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_FIELD_DB-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DB'
...
TOTAL = ZVI_FIELD_DB_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_FIELD_DB-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_FIELD_DB-UPD_FLAG NE SPACE.
