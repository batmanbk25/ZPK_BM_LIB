FUNCTION VIEWFRAME_ZVI_EXCEL_LAYOUT.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_EXCEL_LAYOUT
*   generation date: 22.01.2014 at 19:43:43 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_EXCEL_LAYOUT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_EXCEL_LAYOUT_EXTRACT
TOTAL = ZVI_EXCEL_LAYOUT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_EXCEL_LAYOUT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_EXCEL_LAYOUT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_EXCEL_LAYOUT_EXTRACT
TOTAL = ZVI_EXCEL_LAYOUT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_EXCEL_LAYOUT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_EXCEL_LAYOUT'
...
TOTAL = ZVI_EXCEL_LAYOUT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_EXCEL_LAYOUT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_EXCEL_LAYOUT-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_EXCEL_LAYOUT'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_EXCEL_LAYOUT-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_EXCEL_LAYOUT_EXTRACT
TOTAL = ZVI_EXCEL_LAYOUT_TOTAL
