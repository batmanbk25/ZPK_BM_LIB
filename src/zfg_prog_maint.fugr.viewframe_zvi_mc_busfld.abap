FUNCTION VIEWFRAME_ZVI_MC_BUSFLD.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_MC_BUSFLD
*   generation date: 17.04.2015 at 10:46:23 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSFLD'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_MC_BUSFLD_EXTRACT
TOTAL = ZVI_MC_BUSFLD_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSFLD'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_MC_BUSFLD-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_MC_BUSFLD_EXTRACT
TOTAL = ZVI_MC_BUSFLD_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_MC_BUSFLD-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_MC_BUSFLD'
...
TOTAL = ZVI_MC_BUSFLD_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_MC_BUSFLD-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_MC_BUSFLD-UPD_FLAG NE SPACE.
