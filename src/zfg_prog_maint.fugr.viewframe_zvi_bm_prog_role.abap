FUNCTION VIEWFRAME_ZVI_BM_PROG_ROLE.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_PROG_ROLE
*   generation date: 02.02.2016 at 16:43:41 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_PROG_ROLE'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_BM_PROG_ROLE_EXTRACT
TOTAL = ZVI_BM_PROG_ROLE_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_PROG_ROLE'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_PROG_ROLE-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_PROG_ROLE_EXTRACT
TOTAL = ZVI_BM_PROG_ROLE_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_PROG_ROLE-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_PROG_ROLE'
...
TOTAL = ZVI_BM_PROG_ROLE_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_PROG_ROLE-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_PROG_ROLE-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_PROG_ROLE'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_BM_PROG_ROLE-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_PROG_ROLE_EXTRACT
TOTAL = ZVI_BM_PROG_ROLE_TOTAL
