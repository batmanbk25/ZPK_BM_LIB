FUNCTION ZFM_SCR_FIELD_PROCESS.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
DATA:
  ...
  FIELD-SYMBOLS:
    <LFT_LOOPTAB> TYPE ANY TABLE.       "Loop table of table control
...
*     Process each field on table control
      PERFORM PAI_PROCESS_FIELD_TABCONTROL
        USING LS_FIELD
              LT_FIELD_LOOP
              I_INITIAL
              I_NO_CHECK
              I_WARN_REQUIRED " QuangVN add: Voi du lieu chuyen doi ko check ngay CMND
              T_SCR_CHKSTEP
              I_CPROG
              I_DYNNR.
    ENDIF.
  ...
  T_ERR_FIELD = LT_ERR_FIELD = GT_ERR_FIELD.
  DELETE LT_ERR_FIELD WHERE TYPE <> GC_MTYPE_E.
  IF I_AUTO_STOP IS INITIAL AND LT_ERR_FIELD IS NOT INITIAL.
    DELETE GT_ERR_FIELD WHERE TYPE <> GC_MTYPE_E.
    DELETE T_ERR_FIELD WHERE TYPE <> GC_MTYPE_E.
    E_ERROR = GC_XMARK.
  ENDIF.
