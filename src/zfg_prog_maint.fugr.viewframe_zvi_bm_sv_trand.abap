FUNCTION VIEWFRAME_ZVI_BM_SV_TRAND.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_SV_TRAND
*   generation date: 21.10.2016 at 10:52:55 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRAND'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_BM_SV_TRAND_EXTRACT
TOTAL = ZVI_BM_SV_TRAND_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRAND'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_SV_TRAND-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SV_TRAND_EXTRACT
TOTAL = ZVI_BM_SV_TRAND_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_SV_TRAND-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRAND'
...
TOTAL = ZVI_BM_SV_TRAND_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_SV_TRAND-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_SV_TRAND-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRAND'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_BM_SV_TRAND-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SV_TRAND_EXTRACT
TOTAL = ZVI_BM_SV_TRAND_TOTAL
