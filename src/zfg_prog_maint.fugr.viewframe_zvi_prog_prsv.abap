FUNCTION VIEWFRAME_ZVI_PROG_PRSV.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_PROG_PRSV
*   generation date: 28.10.2014 at 10:44:54 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_PROG_PRSV_EXTRACT
TOTAL = ZVI_PROG_PRSV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_PROG_PRSV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_PROG_PRSV_EXTRACT
TOTAL = ZVI_PROG_PRSV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_PROG_PRSV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSV'
...
TOTAL = ZVI_PROG_PRSV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_PROG_PRSV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_PROG_PRSV-UPD_FLAG NE SPACE.
