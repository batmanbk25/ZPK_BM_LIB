FUNCTION VIEWFRAME_ZVI_BM_SL_SCRVL.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_SL_SCRVL
*   generation date: 17.03.2017 at 00:03:35 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_SL_SCRVL'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_BM_SL_SCRVL_EXTRACT
TOTAL = ZVI_BM_SL_SCRVL_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SL_SCRVL'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_SL_SCRVL-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SL_SCRVL_EXTRACT
TOTAL = ZVI_BM_SL_SCRVL_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_SL_SCRVL-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_SL_SCRVL'
...
TOTAL = ZVI_BM_SL_SCRVL_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_SL_SCRVL-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_SL_SCRVL-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SL_SCRVL'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_BM_SL_SCRVL-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SL_SCRVL_EXTRACT
TOTAL = ZVI_BM_SL_SCRVL_TOTAL
