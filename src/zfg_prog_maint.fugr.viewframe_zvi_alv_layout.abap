FUNCTION VIEWFRAME_ZVI_ALV_LAYOUT.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_ALV_LAYOUT
*   generation date: 27.11.2013 at 23:13:56 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_ALV_LAYOUT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_ALV_LAYOUT_EXTRACT
TOTAL = ZVI_ALV_LAYOUT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_ALV_LAYOUT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_ALV_LAYOUT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_ALV_LAYOUT_EXTRACT
TOTAL = ZVI_ALV_LAYOUT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_ALV_LAYOUT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_ALV_LAYOUT'
...
TOTAL = ZVI_ALV_LAYOUT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_ALV_LAYOUT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_ALV_LAYOUT-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_ALV_LAYOUT'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_ALV_LAYOUT-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_ALV_LAYOUT_EXTRACT
TOTAL = ZVI_ALV_LAYOUT_TOTAL
