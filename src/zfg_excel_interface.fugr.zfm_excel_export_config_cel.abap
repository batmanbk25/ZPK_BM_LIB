FUNCTION ZFM_EXCEL_EXPORT_CONFIG_CEL.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
DATA:
        ...
        LT_RANGES       TYPE ZTT_COPY_RANGES.
  CONSTANTS: GC_RANGES TYPE FIELDNAME VALUE 'RANGES_CONFIG'.

  FIELD-SYMBOLS: <LF_RANGES> TYPE STANDARD TABLE.
...
*   Add row
    LOOP AT T_SQUARE_DATA INTO LS_SQUARE_DATA.
      ...
      OR LS_SQUARE_DATA-INSCL = 'X'.
        CALL FUNCTION 'ZFM_EXCEL_GETDIMENS_CONFIG_CEL'
          IMPORTING
...
*     Merge cells
    ...
    ENDIF.
" ----- Insert by NgocNV8 on 06/11/2014 - Start -----
    " Copy range is configured
    IF I_DATA IS NOT INITIAL AND SY-SUBRC IS INITIAL.
      ASSIGN COMPONENT GC_RANGES OF STRUCTURE I_DATA
        TO <LF_RANGES>.
      IF <LF_RANGES> IS ASSIGNED.
        CLEAR LT_RANGES.
        TRY.
          LT_RANGES[] = <LF_RANGES>[].
        CATCH CX_ROOT.
        ENDTRY.
        CALL FUNCTION 'ZFM_OLE_EXCEL_RANGES_COPY'
          EXPORTING
            I_RANGES    =  LT_RANGES.
      ENDIF.
      CLEAR SY-SUBRC.
    ENDIF.
" ----- Insert by NgocNV8 on 06/11/2014 - End -----
*   Save file
    IF SY-SUBRC IS INITIAL.
...
* Open excel file witk DOI
  ...
  LOOP AT T_SQUARE_DATA INTO LS_SQUARE_DATA.
    CALL FUNCTION 'ZFM_DOI_EXCELEXPORT_CONFIG_CEL'
      EXPORTING
...
*   Set format cells
    IF LS_SQUARE_DATA-CELLS_FORMAT[] IS NOT INITIAL.
      CALL FUNCTION 'ZFM_DOI_EXCEL_FORMAT_CELLS_1'
        EXPORTING
          I_SPREADSHEET        = LO_SPREASHEET
