...
* Check local
  READ TABLE GT_BM_DF_STR INTO LPS_DF_STR
    WITH KEY TABNM = LW_TABNAME BINARY SEARCH.
...
* Aggregate format
  LOOP AT LPS_DF_STR-FIELDS ASSIGNING <LF_NEST_FIELD>.
    ...
    IF SY-SUBRC IS INITIAL.
      ...
      ENDLOOP.
      SORT <LF_NEST_FIELD>-TYPLS BY TYPEID VALUE.
    ENDIF.
  ENDLOOP.

  APPEND LPS_DF_STR TO GT_BM_DF_STR.
  SORT GT_BM_DF_STR BY TABNM.
...
*   Decimal
    WHEN GC_CFORMAT_DEC.
      CREATE DATA LR_DATA LIKE LPW_FIELD.
      ASSIGN LR_DATA->* TO <LF_FIELD>.
      <LF_FIELD> = LPW_FIELD.
      PERFORM DF_FIELD_CHECK_FORMAT_REGEX
        USING LPS_DF_FIELD
              GC_REGEX_DEC
        ...
        CHANGING LPW_FIELD
                 LPS_RETURN
                 LPT_FIELD_EC.
      IF LPS_RETURN-TYPE = 'E'.
        CLEAR: LPS_RETURN.
        LPW_FIELD = <LF_FIELD>.
        PERFORM DF_FIELD_CHECK_FORMAT_REGEX
          USING LPS_DF_FIELD
                GC_REGEX_DEC2
                GC_REGEX_REPLACE_DEC2
                GC_TFORMAT_DEC
          CHANGING LPW_FIELD
                   LPS_RETURN
                   LPT_FIELD_EC.
        IF LPS_RETURN-TYPE <> GC_MTYPE_E.
          DATA: LW_EXPO    TYPE QPMK-SOLLWERT,
                LW_PCKDECI TYPE P LENGTH 16 DECIMALS 14.
          MOVE : LPW_FIELD TO LW_EXPO,
                 LW_EXPO TO LW_PCKDECI,
                 LW_PCKDECI TO LPW_FIELD.
          CONDENSE LPW_FIELD.
        ENDIF.
      ENDIF.
*   Char
...
FORM DF_FIELD_CHECK_LIST
  ...
  READ TABLE LPS_DF_FIELD-TYPLS INTO LS_DF_TYPLS
    WITH KEY VALUE = LPW_FIELD BINARY SEARCH.
  IF SY-SUBRC IS INITIAL.
