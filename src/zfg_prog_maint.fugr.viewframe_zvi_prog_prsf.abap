FUNCTION VIEWFRAME_ZVI_PROG_PRSF.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_PROG_PRSF
*   generation date: 28.10.2014 at 10:44:02 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSF'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_PROG_PRSF_EXTRACT
TOTAL = ZVI_PROG_PRSF_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSF'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_PROG_PRSF-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_PROG_PRSF_EXTRACT
TOTAL = ZVI_PROG_PRSF_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_PROG_PRSF-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_PROG_PRSF'
...
TOTAL = ZVI_PROG_PRSF_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_PROG_PRSF-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_PROG_PRSF-UPD_FLAG NE SPACE.
