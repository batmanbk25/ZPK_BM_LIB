FUNCTION VIEWFRAME_ZVI_FIELD_DESC.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_FIELD_DESC
*   generation date: 20.07.2014 at 01:46:48 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DESC'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_FIELD_DESC_EXTRACT
TOTAL = ZVI_FIELD_DESC_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DESC'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_FIELD_DESC-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_FIELD_DESC_EXTRACT
TOTAL = ZVI_FIELD_DESC_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_FIELD_DESC-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DESC'
...
TOTAL = ZVI_FIELD_DESC_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_FIELD_DESC-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_FIELD_DESC-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_FIELD_DESC'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_FIELD_DESC-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_FIELD_DESC_EXTRACT
TOTAL = ZVI_FIELD_DESC_TOTAL
