FUNCTION VIEWFRAME_ZVI_JBCF_JSTEP.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_JBCF_JSTEP
*   generation date: 06.04.2015 at 16:21:45 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_JBCF_JSTEP'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_JBCF_JSTEP_EXTRACT
TOTAL = ZVI_JBCF_JSTEP_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_JBCF_JSTEP'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_JBCF_JSTEP-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_JBCF_JSTEP_EXTRACT
TOTAL = ZVI_JBCF_JSTEP_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_JBCF_JSTEP-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_JBCF_JSTEP'
...
TOTAL = ZVI_JBCF_JSTEP_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_JBCF_JSTEP-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_JBCF_JSTEP-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_JBCF_JSTEP'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_JBCF_JSTEP-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_JBCF_JSTEP_EXTRACT
TOTAL = ZVI_JBCF_JSTEP_TOTAL
