FUNCTION VIEWFRAME_ZVI_BM_SV_TRANS.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"--------------------------------------------------------------------
*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_SV_TRANS
*   generation date: 21.10.2016 at 10:46:35 by user TUANBA
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRANS'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_BM_SV_TRANS_EXTRACT
TOTAL = ZVI_BM_SV_TRANS_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRANS'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_SV_TRANS-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SV_TRANS_EXTRACT
TOTAL = ZVI_BM_SV_TRANS_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_SV_TRANS-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRANS'
...
TOTAL = ZVI_BM_SV_TRANS_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_SV_TRANS-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_SV_TRANS-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_SV_TRANS'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVI_BM_SV_TRANS-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_SV_TRANS_EXTRACT
TOTAL = ZVI_BM_SV_TRANS_TOTAL
